...
NONMATCH("asm/non_matching/v1EyeLaser/sub_080AB758.inc", void sub_080AB758(Entity* this)) {
    ...
    u32 r7 = 0;
    u32 index = 0;
    if (((u32)this->y.HALF.HI) <= (u16)(gRoomControls.roomOriginY + 0xd4)) {
        u32 tmp = 7;
        do {
            if ((index & tmp) == 7) {
            ...
            index = (index + 1) & tmp;
        } while (this->y.HALF.HI + r7 + (index << 3) <= (u16)(gRoomControls.roomOriginY + 0xd4));
    }
    index = 0;
    if (this->y.HALF.HI + r7 <= (u16)(gRoomControls.roomOriginY + 0xd4)) {
        u32 tmp = 3;
        do {
            if ((index & tmp) == 3) {
